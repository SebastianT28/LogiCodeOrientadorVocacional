    ...
    <div className="min-h-screen bg-[#FDFDFD]">
      ...
      <section className="max-w-[1200px] mx-auto px-8 py-16 grid grid-cols-1 md:grid-cols-2 items-center gap-16 min-h-[calc(100vh-64px)]">
        ...
        <div>
          ...
          <h1 className="font-poppins text-[clamp(2rem,4vw,3rem)] font-bold leading-[1.15] text-[#1C1C1C] mb-5">
        ...
        <div className="relative h-[480px] hidden md:flex items-center justify-center">
          <div className="w-[380px] h-[380px] bg-gradient-to-br from-[#FDF0F0] to-[#fce4e4] rounded-full flex items-center justify-center relative">
            <div className="w-[200px] h-[200px] bg-white rounded-full flex items-center justify-center text-[4rem] shadow-[0_8px_40px_rgba(0,0,0,0.1)] z-[2]">
              🧭
            </div>
          </div>
          {/* Floating cards */}
          <div className="floating-card absolute top-10 -right-2 bg-white rounded-2xl px-4 py-3 shadow-[0_6px_24px_rgba(139,30,30,0.14)] text-sm font-semibold text-[#1C1C1C]"
            style={{ animation: 'float1 4s ease-in-out infinite' }}>🔬 Ciencias de la Salud</div>
          <div className="floating-card absolute bottom-14 -left-5 bg-white rounded-2xl px-4 py-3 shadow-[0_6px_24px_rgba(139,30,30,0.14)] text-sm font-semibold text-[#1C1C1C]"
            style={{ animation: 'float2 4.5s ease-in-out infinite' }}>⚖️ Derecho</div>
          <div className="floating-card absolute bottom-40 -right-8 bg-white rounded-2xl px-4 py-3 shadow-[0_6px_24px_rgba(139,30,30,0.14)] text-sm font-semibold text-[#1C1C1C]"
      ...
      <section className="bg-white py-20 px-8">
        <div className="text-center max-w-[640px] mx-auto mb-12">
          ...
          <h2 className="font-poppins text-[clamp(1.6rem,3vw,2.2rem)] font-bold text-[#1C1C1C] leading-tight mb-4">Un acompañamiento real para una decisión importante</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 max-w-[1000px] mx-auto">
          {[
            { icon: '🧠', title: 'Basado en psicología', text: 'Evalúa intereses, personalidad, valores y motivaciones con fundamento científico.' },
            { icon: '📊', title: 'Dashboard visual', text: 'Gráficos de radar, barras y ranking de afinidad para que veas tus resultados de forma clara.' },
            { icon: '🗺️', title: 'Carreras reales', text: 'Recomendaciones específicas con sedes en Lima y Arequipa.' },
            { icon: '🤝', title: 'Sin presión', text: 'Los resultados son orientaciones, no sentencias. Son un punto de partida.' },
            { icon: '⚡', title: 'Rápido y claro', text: 'Solo 10 minutos para obtener un perfil vocacional completo.' },
          ].map((b) => (
            <div key={b.title} className="bg-[#F5F5F5] rounded-2xl p-6 text-center transition-all duration-300 border border-transparent hover:bg-white hover:border-[#FDF0F0] hover:shadow-md hover:-translate-y-1">
              ...
              <div className="font-poppins font-semibold text-[#1C1C1C] text-sm mb-2">{b.title}</div>
      ...
      <section className="py-20 px-8 bg-[#F5F5F5]">
        <div className="text-center max-w-[640px] mx-auto mb-10">
          ...
          <h2 className="font-poppins text-[clamp(1.6rem,3vw,2.2rem)] font-bold text-[#1C1C1C]">7 áreas del conocimiento</h2>
        </div>
        <div className="flex flex-wrap gap-2.5 justify-center max-w-[800px] mx-auto">
          {[
            ['⚙️', 'Ingeniería'], ['💼', 'Negocios'], ['⚖️', 'Derecho y Ciencias Humanas'],
            ['🏥', 'Ciencias de la Salud'], ['🏛️', 'Arquitectura y Diseño'], ['📡', 'Comunicaciones'], ['📚', 'Educación'],
          ].map(([icon, label]) => (
            <div key={label} className="bg-white border border-[#E0E0E0] rounded-full px-5 py-2 text-sm font-medium text-[#666] flex items-center gap-2 transition-all duration-300 hover:bg-[#8B1E1E] hover:text-white hover:border-[#8B1E1E]">
