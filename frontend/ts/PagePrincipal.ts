document.addEventListener('DOMContentLoaded', (): void => {
    ...
    const menuIcon = document.getElementById('menu-icon') as SVGPathElement | null;
    ...
    // ----------------------------------------
    const accordionHeaders = document.querySelectorAll('.accordion-header') as NodeListOf<HTMLButtonElement>;

    accordionHeaders.forEach((header: HTMLButtonElement) => {
        header.addEventListener('click', () => {
            const item = header.parentElement as HTMLElement | null;
            if (!item) return;

            const content = item.querySelector('.accordion-content') as HTMLElement | null;
            const chevron = header.querySelector('.accordion-chevron') as SVGElement | null;
            if (!content || !chevron) return;

            const isActive = item.classList.contains('active');

            // Cerrar todos los demás acordeones abiertos
            document.querySelectorAll('.accordion-item').forEach((otherItem: Element) => {
                if (otherItem !== item && otherItem.classList.contains('active')) {
                    otherItem.classList.remove('active');
                    const otherContent = otherItem.querySelector('.accordion-content') as HTMLElement | null;
                    const otherChevron = otherItem.querySelector('.accordion-chevron') as SVGElement | null;
